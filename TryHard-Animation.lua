--[[
    ...
    Utilisation : loadstring(game:HttpGet("URL_BRUTE_DE_CE_FICHIER"))()
--]]
...
local unwalkSavedAnimate,_anyKeyListening=false,false
    ...
    if hum and hum.PlatformStand and not activeMedusaBillboard then
        ...
        task.delay(4.5,function() 
            if activeMedusaBillboard then 
                pcall(function() activeMedusaBillboard:Destroy() end)
...
local function setupSpeedIndicator(char)
    ...
    bb.Size=UDim2.new(0,140,0,52)
    bb.StudsOffset=Vector3.new(0,3,0)
    bb.AlwaysOnTop=true
    local discordLabel=Instance.new("TextLabel",bb)
    discordLabel.Size=UDim2.new(1,0,0.4,0)
    discordLabel.BackgroundTransparency=1
    discordLabel.Text=".gg/movee"
    discordLabel.TextColor3=Color3.fromRGB(255,180,210)
...
startBatAimbot=function()
    ...
    aimbotConn=RunService.RenderStepped:Connect(function()
        ...
        local dirToTarget=predictedPos-hrp.Position
...
saveConfig=function()
    local function ks(e)
        if e.kb then 
            return {kb=e.kb.Name,gp=e.gp and e.gp.Name}
        elseif e.gp then 
            return {gp=e.gp.Name}
        else 
    ...
    if writefile then 
        pcall(function() writefile("moveeduels.json",HS:JSONEncode(cfg)) end) 
    end
end
...
local function createStealBar()
    ...
    task.spawn(function()
        ...
        while sbStroke and sbStroke.Parent do
            t=t+0.05
            sbStroke.Transparency=0.05+math.abs(math.sin(t*2))*0.35
    ...
    fillLine.BackgroundColor3=PINK
    ...
    stealSection.Size=UDim2.new(0,110,1,0)
    stealSection.Position=UDim2.new(0,12,0,0)
...
local function destroyMobileButtons()
    if mobGuiRef then 
        pcall(function() mobGuiRef:Destroy() end)
        mobGuiRef=nil 
    end
...
local function buildMobileButtons()
    ...
    panelStroke.Color=Color3.fromRGB(120,40,85)
...
local function buildGui()
    ...
    for _,n in ipairs({"MoveeDuels", "LOL_Ez"}) do
        local old=game:GetService("CoreGui"):FindFirstChild(n)
        if old then old:Destroy() end
        local pg=LP:FindFirstChild("PlayerGui")
        if pg then 
    ...
    mainFrame.BackgroundTransparency=guiTransparencyEnabled and 0.65 or 0
    ...
    bgImage.Image="rbxassetid://136962465833545"
    ...
    local function applyTransparencyToGui(transparent)
        if not transparent then
            ...
            if _transpOriginals then 
                for frame,val in pairs(_transpOriginals) do 
        ...
        if not _transpOriginals then
            ...
            local function snapshot(parent)
                for _,child in ipairs(parent:GetChildren()) do
                    if (child:IsA("Frame") or child:IsA("ScrollingFrame")) and child~=bgImage then 
                        _transpOriginals[child]=child.BackgroundTransparency 
                    end
                    snapshot(child)
                end
            end
        ...
        for frame,_ in pairs(_transpOriginals) do 
            if frame and frame.Parent then 
    ...
    headerFix.Position=UDim2.new(0,0,0,headerH-6)
    ...
    discSmall.Text="discord.gg/movee"
    ...
    tabDivider.Position=UDim2.new(0,0,0,tabBarY+TAB_BAR_H)
    tabDivider.BackgroundColor3=ROW_BORDER
    ...
    for _,name in ipairs(TABS) do
        ...
        sf.Size=UDim2.new(1,0,0,bodyH)
        sf.Position=UDim2.new(0,0,0,bodyY)
        sf.BackgroundTransparency=1
        sf.BorderSizePixel=0
        sf.ScrollBarThickness=2
        sf.ScrollBarImageColor3=PINK3
    ...
    for _,name in ipairs(TABS) do
        ...
        underline.BackgroundColor3=PINK
    ...
    local function mkBoxRow(parent, txt, default, cb)
        ...
        tb.Size=UDim2.new(0,58,0,22)
        tb.Position=UDim2.new(1,-66,0.5,-11)
    ...
    do
        ...
        modeValLbl.Size=UDim2.new(0,60,0,14)
    ...
    laggerBox=mkBoxRow(pg,"Normal Speed",LAGGER_SPEED,function(v) 
        if v>0 and v<=500 then LAGGER_SPEED=v end
        saveConfig() 
    end)
    mkBoxRow(pg,"Carry Speed",LAGGER_CARRY_SPEED,function(v) 
    ...
    mkToggle(pg,"Auto Switch Speed",function(on) 
    ...
    setMedusaResetVisual=mkToggle(pg,"Medusa Reset",function(on) 
    ...
    radInput=mkBoxRow(pg,"Steal Radius",Steal.StealRadius,function(v) 
        if v>=0.5 and v<=300 then Steal.StealRadius=v end
        saveConfig() 
    end)
    durationBox=mkBoxRow(pg,"Steal Duration",Steal.StealDuration,function(v) 
        if v>=0.1 and v<=10 then 
    ...
    setStretchRezVisual=mkToggle(pg,"Stretch Rez",function(on) 
        if on then enableStretchRez() else disableStretchRez() end
        saveConfig() 
    end)
    mkToggle(pg,"Ragdoll GUI",function(on) 
    ...
    do
        ...
        local skyIndex=1
        for i,theme in ipairs(SkyOrder) do 
            if theme==currentSkyTheme then 
                skyIndex=i
                break 
            end 
        end
        local skyVal=Instance.new("TextLabel",row)
        skyVal.Size=UDim2.new(0,70,0,22)
        skyVal.Position=UDim2.new(1,-120,0.5,-11)
        skyVal.BackgroundTransparency=1
        skyVal.Text=currentSkyTheme
        skyVal.TextColor3=PINK
        ...
        cycleBtn.Size=UDim2.new(0,40,0,22)
        cycleBtn.Position=UDim2.new(1,-48,0.5,-11)
        cycleBtn.BackgroundColor3=PINK3
    ...
    do
        ...
        for idx,v in ipairs(fovOptions) do 
            if v==fovValue then fovIndex=idx end 
        end
    ...
    do
        ...
        resetBtn.Size=UDim2.new(0,58,0,22)
        resetBtn.Position=UDim2.new(1,-66,0.5,-11)
        resetBtn.BackgroundColor3=Color3.fromRGB(140,25,65)
    ...
    setTranspVisual=mkToggle(pg,"GUI Transparent",function(on) 
        guiTransparencyEnabled=on
        applyTransparencyToGui(on)
        saveConfig() 
    end)
    if guiTransparencyEnabled then 
        setTranspVisual(true)
        applyTransparencyToGui(true) 
    end
        ...
        if mobGuiRef then
            for _,child in ipairs(mobGuiRef:GetDescendants()) do
                if child:IsA("Frame") and child:GetAttribute("BtnKey") then
                    ...
                    if uic2 then 
                        local BS=math.floor(mobileButtonsSize*0.55)
                        uic2.CornerRadius=on and UDim.new(1,0) or UDim.new(0,math.max(8,math.floor(BS*0.21))) 
                    end
                end
            end
        end
    ...
    if circleButtonsEnabled then 
        task.defer(function() 
            if setCircleBtnsVisual then setCircleBtnsVisual(true) end 
        end) 
    end
    mkToggle(pg,"Auto Swing (Move)",function(on) 
        autoMoveSwingEnabled=on
        saveConfig() 
    end)
    mkBoxRow(pg,"Swing Speed (s)",autoMoveSwingInterval,function(v) 
        if v>=0.05 and v<=2 then autoMoveSwingInterval=v end
        saveConfig() 
    end)
    mkToggle(pg,"Btn Drag Per Btn",function(on) 
